 ...
 * Uses title as filename, with issue number as prefix for easy cross-referencing
 */
export function generateIssueFilename(issue: GitHubIssue): string {
	const sanitizedTitle = sanitizeFilename(issue.title);

	// Issue number as prefix for easy cross-reference with GitHub
	// Format: "(123) Issue Title"
	return `(${issue.number}) ${sanitizedTitle}`;
}
